import React,{useState} from 'react';
import './PaymentTab.css'

const PaymentTab = ({value:paymentDetails,onChild5}) => {
    const [number , setNumber] = useState('')
    const [name , setName] = useState('')
    const [cvv , setCvv] = useState('')
    const [month , setMonth] = useState('')
    const [year , setYear] = useState('')


    const handleNumber=(e)=>{
        e.preventDefault()
        let value=e.target.value
        if(!value){
            return(setNumber("Card Number is Required"))
        }
        if(value){
            let regExp=/^([0-9]{16})$/
            if(!(regExp.test(value))){
                return(setNumber("Card number should be of 16 digits"))
            }
            else{
                return(setNumber(""))
                
            }
        }
    }
    const handleName=(e)=>{
        e.preventDefault()
        let value=e.target.value
        if(!value){
            return(setName(" Name is Required"))
        }else{
            return(setName(""))
        }
    }
    const handleCVV=(e)=>{
        e.preventDefault()
        let value=e.target.value
        if(!value){
            return(setCvv("CVV is Required"))
        }
        if(value){
            let regExp=/^([0-9]{3})$/
            if(!(regExp.test(value))){
                return(setCvv("CVV not valid"))
            }
            else{
                return(setCvv(""))
                
            }
        }
    }
    const handleMonth=(e)=>{
        e.preventDefault()
        let value=e.target.value
        if(!value){
            return(setMonth("Month is Required"))
        }
        if(value){
            let regExp=/^01|02|03|04|05|06|07|08|09|10|11|12$/
            if(!(regExp.test(value))){
                return(setMonth("Month not valid"))
            }
            else{
                return(setMonth(""))
                
            }    
        }
    }
    const handleYear=(e)=>{
        e.preventDefault()
        let value=e.target.value
        if(!value){
            return(setYear("Year is Required"))
        }
        if(value){
            let regExp=/^(20[2-9]\d|3000)$/
            if(!regExp.test(value)){
                return(setYear("year should be between 2020 - 3000"))
            }
            else{
                return(setYear(""))
            }
        }
    }
    const handleSubmit=(e)=>{
        if(onChild5){
            let e={n:4}
            onChild5(e)
        }
    }
    const handleBack=(e)=>{
        e.preventDefault()
        if(onChild5){
            let e={n:1}
            onChild5(e)
        }
    }
    
    return (
        <div>
            <br/>
            <div className="amount row">
                <p className = "col-7" >Total Payable Amount is Rs.{paymentDetails}</p>
                <p className = "col-5">Transaction Id:{Math.floor(Math.random() * (2000000 - 1000000) + 1000000)}</p>
            </div>
            <form onSubmit={e=>handleSubmit(e)}> 
                <div className="container">
                    <div className="row credit">
                        <div className="col-4"><b>Card Type : </b> &nbsp;&nbsp;</div>
                        <div className="col-6">
                            <input type="radio" name="card" value= {"credit"}  />&nbsp;<b>Credit Card</b>  &nbsp;
                            <input type="radio" name="card" value={"debit"} />&nbsp;<b>Debit Card</b>  &nbsp;
                        </div>
                    </div>
                    <br/>
                    <div className = "row">
                        <div className = "col-4"><label htmlFor="number"><b>Card Number</b> :</label> </div>
                        <div className = "col-6">
                            <input type="number" placeholder="Enter Card Number" name="number" onBlur={e=>{handleNumber(e)}}  />
                            <div style={{color: "red"}}>{number}</div>
                        </div>
                    </div>
                    <br/>
                    <div className = "row">
                        <div className = "col-4"><label htmlFor="name"><b>Name On Card </b> :</label> </div>
                        <div className = "col-6">
                            <input type="text" placeholder="Enter Card Holder Name" name="name" onBlur={e=>{handleName(e)}}  />
                        <div style={{color: "red"}}>{name}</div>
                        </div>
                    </div>
                    <br/>
                    <div className = "row">
                        <div className = "col-4"><label htmlFor="cvv"><b>CVV</b> :</label> </div>
                        <div className = "col-6">
                            <input type="number" placeholder="Enter CVV" name="cvv" onBlur={e=>{handleCVV(e)}}  />
                            <div style={{color: "red"}}>{cvv}</div>
                        </div>
                    </div>
                    <br/>
                    <div className = "row">
                        <div className = "col-4"><label htmlFor="date"><b>Expiry Date</b> :</label> </div>
                        <div className = "col-3">
                            <input type="number" placeholder="Enter Month" name="date" onBlur={e=>{handleMonth(e)}}  />
                            <div style={{color: "red"}}>{month}</div>
                        </div>
                        <div className = "col-3">
                            <input type="number" placeholder="Enter Year" name="date" onBlur={e=>{handleYear(e)}}  />
                            <div style={{color: "red"}}>{year}</div>
                        </div>
                    </div>
                    <div className="row">
                        <div className="col-3"></div>
                        <div className="col-3"><button className="btn btn-success" type="submit"  >Pay Amount</button></div>
                        <div className="col-3"><button className="btn btn-danger" onClick={e=>handleBack(e)}  >Cancel</button></div>
                        <div className="col-3"></div>
                    </div>
                    

                    
                </div>

            </form>
        </div>
    );
};

export default PaymentTab;